...
// intentionally skip encryption/decryption at certian rate
CryptoEngine.skipRate = function (n) {
	this._skip = n;
};
...
CryptoEngine.prototype.decrypt = function __ceDecrypt(cipherKeyIn, cipherNonceIn, macKeyIn, seq, payload) {
	...
	// decrypt
	return ctr(cipher, cipherNonce, seq, cipherText, this._skip);
};
...
CryptoEngine.prototype.encrypt = function __ceEncrypt(cipherKeyIn, cipherNonceIn, macKeyIn, useq, payload) {
	...
	var macKey = toBuffer(macKeyIn);
	var cipher = createCipher(cipherKey);
	var cipherText = ctr(cipher, cipherNonce, useq, payload, this._skip);
...
function ctr(cipher, cipherNonce, seq, input, skipRate) {
	if (input.length % skipRate !== 0) {
		return input;
	}
